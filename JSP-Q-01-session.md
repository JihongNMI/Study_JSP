...
🤷‍♀️음...... 함수? 메소드? jsp 측면에서 일단 내가 생각한걸 말해볼테니 fact check등 해서 좀 다듬어볼래
1. [로그인했거나 안한 분기가 있는 메인] : session.getAtrribute("user_id")가 null인지 아닌지로 분기를 해서 이걸로 ~님 반갑습니다를 출력합니다.
2. [로그인] : get이나 post를 통해서 정보를 받는데 보통 form action="loginProcess.jsp"의 post방식으로, 여기서의 name으로 id, pw를 받습니다. 그리고 이 페이지에서 request.getParameter("loginErr")의 값을 스트링에 받아와서, 이 값을 비교해서 로그인 성공 실패를 검증합니다. loginErr는 loginProcess.jsp에서 다룹니다.
3. [로그인프로세스] : 여기에선 스트링에 2에서 받아온 id, pw를 request.getParameter("id"), request.getParameter("pw")로 저장합니다. 그리고 맞는지 확인 한 후에 맞다면, session.setAttribute("user_id", id);를 통해서 1에서 쓸 '세션의 user_id'를 채워 넣고, response.sendRedirect("index.jsp");를 통해서 페이지를 돌려줍니다. 만약 틀렸다면, request.getRequestDispatcher("login.jsp?loginErr=1") .forward(request, response);를 써서 돌려줍니다
