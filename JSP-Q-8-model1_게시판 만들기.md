...
editProcess.jsp는 거의 같으니 문제없다. edit.jsp가 문제인데

edit는 기본적으로 원래 notice_view.jsp를 토대로 해서, 필요없는 코드는 지우고 edit에서 쓰는 코드를 붙여넣어서 조합해야한다

1. 조회수1증가 : [삭제] -> 세션이랑 dto랑 비교해서, 작성자 본인만 수정가능 jsfuntion메세지 출력 [추가]
2. 삭제 스크립트 : [삭제] (활용해도 되지만) -> 제목이나 내용 없으면 입력하라고 하는 스크립트 [추가]
3. 원래 보여주는 곳을 form으로 감싼다(값을 보내야되니까) [추가]
4. input이랑 textarea로 교체해서 만든다 [교체]
5. 원래 session이 있으면 수정&삭제 버튼 보여주는게 있었는데 여긴 필요없으니 [삭제]
6. <!-- hidden : 보이지 않지만 submit실행시 데이터를 함께 전송하는 타입 --> <input type="hidden" name="num" value="<%=num %>"/> 이 부분은 그대로 둔다. num 보낼려고 하는거겠지? [유지]
