...
3. 원래 보여주는 곳을 form으로 감싼다(값을 보내야되니까)
4. input이랑 textarea로 교체해서 만든다
5. 원래 session이 있으면 수정&삭제 버튼 보여주는게 있었는데 여긴 필요없으니 [삭제]
6. <!-- hidden : 보이지 않지만 submit실행시 데이터를 함께 전송하는 타입 --> <input type="hidden" name="num" value="<%=num %>"/> 이 부분은 그대로 둔다. num 보낼려고 하는거겠지?
