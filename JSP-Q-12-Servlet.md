...
좋아요! 각 기능 이름 옆에 영어 단어를 추가해서 표로 만들어드릴게요.

| 기능 (Function)            | doGet                    | doPost                                |
| ------------------------ | ------------------------ | ------------------------------------- |
| 게시글 목록 조회 (List)         | DB에서 전체 게시물 조회 → JSP 출력  | ✖                                     |
| 게시글 상세보기 (View)          | 특정 게시물 조회 → JSP 출력       | ✖                                     |
| 게시글 작성 폼 이동 (Write Form) | 작성 폼 JSP 호출              | ✖                                     |
| 게시글 작성 처리 (Write)        | ✖                        | 폼 데이터 수신 → DB 삽입 → 목록 페이지 리다이렉트       |
| 게시글 수정 폼 이동 (Edit Form)  | 수정 대상 게시물 조회 → 수정 JSP 전달 | ✖                                     |
| 게시글 수정 처리 (Edit)         | ✖                        | 폼 데이터 수신 → DB 업데이트 → 상세보기 리다이렉트       |
| 게시글 삭제 처리 (Delete)       | ✖                        | 게시물 삭제 → 목록 페이지 리다이렉트                 |
| 로그인 폼 이동 (Login Form)    | 로그인 JSP 호출               | ✖                                     |
| 로그인 처리 (Login)           | ✖                        | 사용자 인증 → 세션에 UserId 저장 → 메인 페이지 리다이렉트 |
| 로그아웃 (Logout)            | 세션 무효화 → 로그인 페이지 이동      | ✖                                     |

---

💡 포인트

* 괄호 안 영어 단어는 **보통 서블릿/서비스 이름이나 기능 이름**으로 사용
* 조회는 doGet, 상태 변경은 doPost

원하면 제가 이걸 **간단한 게시판 흐름 그림 + doGet/doPost 연결**까지 해서 시각화해 보여드릴 수도 있어요.
