...
기능을 행으로 열은 doget dopost로 해서 다시 만들어줄래

좋아요! 요청하신 대로 **기능을 행으로, `doGet`/`doPost`를 열로** 정리한 표를 만들어드릴게요.

| 기능          | doGet                    | doPost                                |
| ----------- | ------------------------ | ------------------------------------- |
| 게시글 목록 조회   | DB에서 전체 게시물 조회 → JSP 출력  | ✖                                     |
| 게시글 상세보기    | 특정 게시물 조회 → JSP 출력       | ✖                                     |
| 게시글 작성 폼 이동 | 작성 폼 JSP 호출              | ✖                                     |
| 게시글 작성 처리   | ✖                        | 폼 데이터 수신 → DB 삽입 → 목록 페이지 리다이렉트       |
| 게시글 수정 폼 이동 | 수정 대상 게시물 조회 → 수정 JSP 전달 | ✖                                     |
| 게시글 수정 처리   | ✖                        | 폼 데이터 수신 → DB 업데이트 → 상세보기 리다이렉트       |
| 게시글 삭제 처리   | ✖                        | 게시물 삭제 → 목록 페이지 리다이렉트                 |
| 로그인 폼 이동    | 로그인 JSP 호출               | ✖                                     |
| 로그인 처리      | ✖                        | 사용자 인증 → 세션에 UserId 저장 → 메인 페이지 리다이렉트 |
| 로그아웃        | 세션 무효화 → 로그인 페이지 이동      | ✖                                     |
...
* 조회 전용 기능 → **doGet**
* DB 상태 변경, 세션 변경 → **doPost**
* 삭제 처리와 같은 위험 작업도 반드시 **doPost 전용**, doGet에서는 처리하지 않음

원하면 제가 이 표를 **게시판 흐름 그림**과 함께 시각화해서 보여드릴 수도 있어요.
그림으로 보면 doGet/doPost 호출 흐름이 한눈에 보입니다.
